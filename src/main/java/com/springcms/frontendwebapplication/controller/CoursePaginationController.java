package com.springcms.frontendwebapplication.controller;

import java.util.Collection;

import javax.servlet.http.HttpServletRequest;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;

import com.springcms.frontendwebapplication.entity.Course;
import com.springcms.frontendwebapplication.service.CourseRestService;

@Controller
@RequestMapping("/coursesPageView")
public class CoursePaginationController {
	
	@Autowired
	private CourseRestService courseService;
	
	@GetMapping("/courseCatalog/{pageId}")
	public String courseCatalogPageView(@PathVariable int pageId, Model model, HttpServletRequest request) {
		int total = 3;
		
		if(pageId == 1) {}
		else {
			pageId = (pageId - 1) * total+1;
		}
		
		Collection<Course> courses = courseService.getCoursesByPage(pageId, total, request);
		
		model.addAttribute("courses", courses);
		
		return "course-catalog";
	}
}
